...
import org.ergoplatform.dex.protocol.amm.{ParserVersion, N2DexyCFMMTemplates => templates}
...
final class N2DexyOrdersV1Parser[F[_]: Applicative](ts: Long)(implicit
                                                           e: ErgoAddressEncoder
) extends CFMMOrdersParser[N2Dexy_CFMM, ParserVersion.V1, F] {
...
object N2DexyOrdersV1Parser {

  def make[F[_]: Monad: Clock](implicit e: ErgoAddressEncoder): CFMMOrdersParser[N2Dexy_CFMM, ParserVersion.V1, F] =
    now.millis.map(ts => new N2DexyOrdersV1Parser(ts): CFMMOrdersParser[N2Dexy_CFMM, ParserVersion.V1, F]).embed
}
