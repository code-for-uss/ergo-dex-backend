...
import org.ergoplatform.dex.tracker.parsers.amm.pools.N2TCFMMPoolsParser
...
object ExecSwap extends IOApp with SigmaPlatform {
  ...
  val monetaryConfig: MonetaryConfig = MonetaryConfig(1000000L, 0L, 60000L, 1)

  implicit val resolve = new DexOutputResolver[IO] {
    def getLatest: IO[Option[Output]] = IO(None)

    def setPredicted(output: Output): IO[Unit] = IO.unit

    def invalidateAndUpdate: IO[Unit] = IO.unit
  }
