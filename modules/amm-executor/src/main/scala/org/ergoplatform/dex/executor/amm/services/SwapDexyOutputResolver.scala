...
trait SwapDexyOutputResolver[F[_]] {
...
object SwapDexyOutputResolver {
  ...
  def make[I[_]: Sync, F[_]: Sync](
                                    ...
                                    logs: Logs[I, F],
                                    iso: IsoK[F, I]
                                  ): I[SwapDexyOutputResolver[F]] =
    logs.forService[SwapDexyOutputResolver[F]].flatMap { implicit __ =>
      Ref.in[I, F, Option[Output]](null).flatMap { ref =>
        iso
          ...
          .map { _ =>
            new Tracing[F] attach new InMemory[F](tokenId, ref): SwapDexyOutputResolver[F]
          }
      }
    }
  ...
  final private class InMemory[F[_]: Monad](tokenId: TokenId, cache: Ref[F, Option[Output]])(implicit
                                                                                             explorer: ErgoExplorer[F]
  ) extends SwapDexyOutputResolver[F] {
  ...
  final private class Tracing[F[_]: Monad: Logging] extends SwapDexyOutputResolver[Mid[F, *]] {
