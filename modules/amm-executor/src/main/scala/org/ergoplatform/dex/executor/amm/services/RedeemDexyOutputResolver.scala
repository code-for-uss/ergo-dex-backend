...
trait RedeemDexyOutputResolver[F[_]] {
...
object RedeemDexyOutputResolver {
  ...
  def make[I[_]: Sync, F[_]: Sync](
                                    ...
                                    logs: Logs[I, F],
                                    iso: IsoK[F, I]
                                  ): I[RedeemDexyOutputResolver[F]] =
    logs.forService[RedeemDexyOutputResolver[F]].flatMap { implicit __ =>
      Ref.in[I, F, Option[Output]](null).flatMap { ref =>
        iso
          ...
          .map { _ =>
            new Tracing[F] attach new InMemory[F](tokenId, ref): RedeemDexyOutputResolver[F]
          }
      }
    }
  ...
  final private class InMemory[F[_]: Monad](tokenId: TokenId, cache: Ref[F, Option[Output]])(implicit
                                                                                             explorer: ErgoExplorer[F]
  ) extends RedeemDexyOutputResolver[F] {
  ...
  final private class Tracing[F[_]: Monad: Logging] extends RedeemDexyOutputResolver[Mid[F, *]] {
